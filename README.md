# Vitor Rodrigues backbase

Install protractor 1

npm install -g protractor
webdriver-manager update
Install reporting ( local installation ):

cd ./confs/ and npm install command from the link below: 1 - html reporter
Run:

run cmd and start webdriver: webdriver-manager start and leave the server running
again run cmd --> cd ./confs/
run protractor conf.js
When tests are finished, if necessary we can generate a html report If there are any failures, the page screenshots are saved.
